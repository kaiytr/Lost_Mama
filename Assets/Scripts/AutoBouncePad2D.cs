...
public class AutoBouncePad2D : MonoBehaviour
{
    ...
    [Header("Integration")]
    [SerializeField] private float protectPlayerMoveSeconds = 0.12f; // ✅ 바운스 직후 PlayerMove가 X를 덮어쓰지 않게 보호
    ...
    private void TryLaunch(Rigidbody2D rb)
    {
        ...
        // =========================
        // ✅✅ 여기 1줄이 핵심: 바운스 직후 PlayerMove가 X를 덮어쓰지 못하게 잠깐 보호
        // =========================
        if (rb.TryGetComponent<PlayerMove>(out var pm))
            pm.NotifyExternalVelocity(protectPlayerMoveSeconds);
